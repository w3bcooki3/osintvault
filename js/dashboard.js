...
function updateDashboard() {
    ...
    // --- Calculate entries added today and this week ---
    const today = new Date();
    today.setHours(0, 0, 0, 0); // Start of today

    const firstDayOfWeek = new Date(today);
    // Adjust to Monday if your week starts on Monday, otherwise Sunday (today.getDay() === 0)
    // Sunday (0), Monday (1), ..., Saturday (6)
    // To get Monday: (today.getDay() + 6) % 7
    firstDayOfWeek.setDate(today.getDate() - (today.getDay() + 6) % 7); // This calculates Monday of the current week
    firstDayOfWeek.setHours(0, 0, 0, 0);

    let entriesAddedToday = 0;
    let entriesAddedThisWeek = 0;
    ...
    allEntries.forEach(entry => {
        if (entry.addedDate) {
            ...
            // Check if entryDate is within the current week (from Monday to Sunday of current week)
            if (entryDate.getTime() >= firstDayOfWeek.getTime() && entryDate.getTime() <= today.getTime()) {
                entriesAddedThisWeek++;
            }
        }
    });

    document.getElementById('entriesAddedTodayCount').textContent = entriesAddedToday;
    document.getElementById('entriesAddedThisWeekCount').textContent = entriesAddedThisWeek;
    ...
    // Case Studies Count
    document.getElementById('caseStudiesCount').textContent = appState.caseStudies.length;

    // Dork Query Templates Count
    // Assuming dorkTemplates is a global object from dorkAssistant.js
    const totalDorkTemplates = Object.values(dorkTemplates).flat().length;
    document.getElementById('dorkTemplatesCount').textContent = totalDorkTemplates;
    ...
    document.getElementById('dorksSummaryTemplates').textContent = totalDorkTemplates;
 ...
 * Shows data for the past 6 weeks.
...
function renderToolsAddedPerWeek() {
    ...
    // Show only the past 6 weeks
    const weeksToShow = sortedWeeks.slice(0, 6);
